...
public class E101 {
				...
				for (String s : line.split("[, \n ; ]")) {
					if(s.length() > 2) {
						if(!words.contains(s.toUpperCase())) {
							...
							System.out.println("Word list contains duplicated words. Aborting execution");
							sopa.close();
							words.add("-1");
							return words;
						}
					...
					if (s.length() < 3 && !s.equals("")) {
						System.out.println("Word '" + s + "' is smaller than 3 characters. "
								+ "Aborting execution...");
						sopa.close();
						words.add("-1");
						return words;
					...
					if (!s.matches("[a-zA-Z]+") && !s.equals("")) {
						System.out.println("Word '" + s + "' contains non-alphabetic characters. "
								+ "Aborting execution...");
						sopa.close();
						words.add("-1");
						return words;
					...
					if (s.matches("[A-Z]+") && !s.equals("")) {
						System.out.println("Word '" + s + "' only contains upper case characters. "
								+ "Aborting execution...");
						sopa.close();
						words.add("-1");
						return words;
	...
	public static int gridchecker(String filename) throws IOException {
			...
			if(!line.matches(".*[, ;].*")) {
				...
				for(char c : letters) {
					if(Character.isLowerCase(c)) {
						System.out.println("Grid contains lower case letter. Aborting execution...");
						sopa.close();
						return 1;
					}
					if(!Character.isAlphabetic(c)) {
						System.out.println("Grid contains non-alphabetic character. Aborting execution...");
						sopa.close();
						return 1;
					}
				}
		...
		if(ln > 60 || width > 60) {
			System.out.println("Grid size exceeds maximum of 60x60. Aborting execution...");
			sopa.close();
			return 1;
		}
		if(ln+1 != width) {
			System.out.println("Grid is not square. Aborting execution...");
		}
		sopa.close();
		return 0;
	}
