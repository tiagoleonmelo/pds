...
public class JGalo extends JFrame implements ActionListener {
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource().getClass().getSimpleName().equals("JToggleButton")){
			//Colocar no butao o marcador do jogador
		...
		for (int i=0; i<9; i++)
			if (e.getSource() == bt[i]) {
				jogo.setJogada(i/3+1, //nº da linha (1,1,1,2,2,2,3,3,3)
							   i%3+1);//nº da coluna(1,2,3,1,2,3,1,2,3)
			}
	...
	public static void main(String args[]) {
		if (args.length >= 1)
			new JGalo( new JogoGaloEngine(3, args[0].charAt(0)) );
		else
			new JGalo( new JogoGaloEngine(3) );
	}
} 
